...
When default backend ports are occupied by another worktree, start the local backend on alternate ports with `scripts/dev-up.sh --services-only` and start Vite with the same `USER_API_PORT`, `AUTH_API_PORT`, `FRIENDS_API_PORT`, `MESSAGE_API_PORT`, `GATEWAY_WS_PORT`, and `GROUPS_API_PORT` environment variables. The local message chain runs through Redpanda (started by `docker compose up -d redpanda` / `dev-up.sh`): msg-rpc publishes to Kafka and msgtransfer consumes, persists, and dispatches to `msggateway`.
