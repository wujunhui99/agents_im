...
def parser() -> argparse.ArgumentParser:
    ...
    rec.add_argument("--output", default=str(DEFAULT_OUTPUT), help=f"JSONL output path, default {DEFAULT_OUTPUT}")
    rec.add_argument("--repo", default=DEFAULT_REPO)
    rec.add_argument("--task-id", required=True, help="Hermes/controller task id, e.g. codex-issue-236-feedback-ui-admin")
    rec.add_argument("--agent", default="codex")
    rec.add_argument("--codex-session-id")
    rec.add_argument("--issue", required=True, help="GitHub issue number, #number, or issue URL")
    rec.add_argument("--branch")
    rec.add_argument("--worktree")
    rec.add_argument("--commit")
    rec.add_argument("--outcome", required=True, choices=("success", "blocked", "failed", "partial", "cancelled"))
    rec.add_argument("--started-at", help="ISO-8601 UTC timestamp")
    rec.add_argument("--ended-at", help="ISO-8601 UTC timestamp")
    rec.add_argument("--duration", help="seconds or compact duration like 1h20m")
    rec.add_argument("--tokens-used", type=int)
    rec.add_argument("--pr-url")
    rec.add_argument("--evidence", action="append", help="repeatable; command, URL, commit, or verification evidence")
    rec.add_argument("--blocker", action="append", help="repeatable; blocker that delayed the task")
    rec.add_argument("--major-time-sink", action="append", help="repeatable; main time sink even if not a hard blocker")
    rec.add_argument("--permissions-help", choices=("yes", "no", "unknown"), help="would more access/permission likely improve speed?")
    rec.add_argument("--permission-candidate", action="append", help="repeatable; e.g. database, k8s, drone-ci, object-storage")
    rec.add_argument("--permission-reason", help="why the permission would/would not improve speed")
    rec.add_argument("--lesson", action="append", help="repeatable; pitfall or reusable lesson")
    rec.add_argument("--notes")
    rec.add_argument("--publish", action="store_true", help="publish report to Management System after local JSONL write")
    rec.add_argument("--publish-url", help=f"Management System task-report API URL; env HERMES_TASK_REPORT_PUBLISH_URL or default {DEFAULT_PUBLISH_URL}")
    rec.add_argument("--publish-token", help="admin bearer token; env HERMES_TASK_REPORT_PUBLISH_TOKEN")
    rec.add_argument("--publish-timeout", type=int, default=15, help="publish timeout seconds")
