...
# agent-rpc goctl model 数据层（conversation_ai_hosting AG-6①、registry #605）：直连 PG。
# 覆盖 bigint 列读出为 string ID 的 ::text 边界，捕获 #531/#550 类 bigint=text 回归。
go test -tags=integration -timeout 180s ./service/agent/rpc/...
