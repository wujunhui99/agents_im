...
Prometheus and Grafana are deployed by k3s from `deploy/k8s/prometheus-grafana.yaml`. Prometheus is exposed only through the Management System path `https://ms.agenticim.xyz/observability/metrics` behind the `observability-basic-auth` Traefik middleware; do not expose `prometheus.agenticim.xyz`. Grafana is exposed at `https://grafana.agenticim.xyz/` and uses its own login backed by the `agents-im/grafana-admin` Secret. `scripts/bootstrap-server.sh` creates that Secret for new servers and stores the generated operator credentials at `/opt/agents-im/grafana-admin.env` with mode `0600`. Keep both credential files and k3s Secret values out of Git and chat logs.
...
- Prometheus: `https://ms.agenticim.xyz/observability/metrics` (Traefik basic-auth; no separate `prometheus.agenticim.xyz` ingress)
...
The Management System root serves the web SPA and the frontend opens **AgenticIM Management** from that host. On `ms.agenticim.xyz`, Ingress exposes the admin API prefixes served by `message-api` (`/admin/dashboard`, `/admin/llm-traces`, `/admin/conversations`, `/admin/users`, `/api/admin/feedback`, `/api/admin/task-reports`) plus `/` to `web`. User feedback submission uses `/api/feedback`, feedback management data uses `/api/admin/feedback`, and Hermes/Codex task reports use `/api/admin/task-reports` so SPA routes such as `/feedback`, `/admin/feedback`, and `/admin/task-reports` can render pages without conflicting with JSON fetches. AgenticIM Management provides operator shortcuts whose paths are real routes: `/observability/logs` redirects to Grafana Explore with Loki, `/observability/traces` redirects to Grafana Explore with Tempo, `/observability/metrics` serves the protected Prometheus UI directly, and `/observability/llm` redirects to the independent `https://langfuse.agenticim.xyz/` domain. Grafana Explore redirects must use the `schemaVersion=1&panes=...` URL format with explicit datasource UID/type (`loki`, `tempo`) instead of the legacy `left={"datasource":...}` format, because Grafana may otherwise fall back to the user's previous/default Explore datasource. Loki and Tempo keep no public domains, `prometheus.agenticim.xyz` must remain unexposed, and Langfuse keeps its independent domain. `admin.agenticim.xyz` is deprecated and has a Traefik permanent redirect to the same path on `https://ms.agenticim.xyz`; do not use it as the primary management entrypoint.
