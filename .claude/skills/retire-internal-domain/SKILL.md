...
|----|----|-----------|--------|------|
| media | #401 | `service/media/core` | `internal/mediavalidate`（message 发送校验 + user 头像校验）| 数据层仍用 internal/repository |
| friends | #402 | `service/friends/core` | `core.AccountRepoUserLookup`（直读 account 仓库映射 UserProfile，待 user-rpc 化）| 未被 monolith 消费；定义本地 `core.UserProfile`/`UserLookup`；e2e harness 同步改指 core |
