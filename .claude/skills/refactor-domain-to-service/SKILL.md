...
- **message monolith 是 keystone**：AI 托管 runtime（旧 `internal/servicecontext/message`，#341/A4 起重定位到
  `service/msg/rpc/internal/aihosting`）会 in-process 构造几乎所有域的 `*Logic`（消费方现仅剩 msg-rpc——#463 后；msggateway 已切 gRPC 脱钩，#492）。被它消费的域，
