...
- Hermes/Codex 协作模型：[`docs/agent-ops/hermes-codex-operating-model.md`](./docs/agent-ops/hermes-codex-operating-model.md)
- Codex 交付契约：[`docs/agent-ops/codex-worker-handoff.md`](./docs/agent-ops/codex-worker-handoff.md)
- Controller 验收与发布清单：[`docs/agent-ops/controller-review-and-release.md`](./docs/agent-ops/controller-review-and-release.md)
...
当前关键事实：`main` push 通过 Drone 触发 deploy；Drone 地址是 `https://drone.agenticim.xyz`；Drone repo secrets 只记录名称不记录值，详见 `deploy/README.md`；k3s namespace 是 `agents-im`；可使用 SSH alias `server-ssh-tls` 但不要暴露真实连接信息；CI 绿不等于 runtime 绿，必要时查 pod/log/rollout/live API/WS。

### Codex worker / Controller handoff

Codex 或其他实现 worker 接到非平凡任务时必须读：

- [`docs/agent-ops/hermes-codex-operating-model.md`](./docs/agent-ops/hermes-codex-operating-model.md)
- [`docs/agent-ops/codex-worker-handoff.md`](./docs/agent-ops/codex-worker-handoff.md)
- 对应任务专题 docs

Hermes/Controller 验收 worker 结果、合并和发布时必须读：

- [`docs/agent-ops/controller-review-and-release.md`](./docs/agent-ops/controller-review-and-release.md)

关键事实：Codex exit 0、自述 done、或提交了 PR 都不是验收；Controller 必须复核 diff、测试和必要线上证据。功能 PR 合到 `develop` 不是生产完成；影响线上行为时还要 `develop -> main` promotion 和生产 smoke。
