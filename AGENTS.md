...
当前关键事实：`main` push 通过 Drone 触发 deploy；k3s namespace 是 `agents-im`；可使用 SSH alias `server-ssh-tls` 但不要暴露真实连接信息；CI 绿不等于 runtime 绿，必要时查 pod/log/rollout/live API/WS。
