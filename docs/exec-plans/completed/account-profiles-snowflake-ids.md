...
- Agent 绑定账号时只依赖 `account_type=agent`，不依赖 ID 前缀。
...
- PostgreSQL schema 改为 `accounts` + `profiles`；`agents.account_id` 引用 `accounts(account_id)`，Agent 展示资料依赖 `profiles` 和 `account_type=agent`。
