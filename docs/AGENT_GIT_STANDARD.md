...
3. **PR 到 main 并通过 Merge Queue**：本项目采用 `feature/fix/ci/... -> main` 的 GitHub Merge Queue 流程；不再把 `develop` 作为日常集成分支。
...
本项目采用 **one issue -> one PR -> main Merge Queue closes issue**：

- 每个开发分支必须对应一个 GitHub Issue。
- 每个开发 PR 默认只解决一个 Issue；不要把多个 Issue 混在一个 PR 里。
- PR 目标分支是 `main`，并且必须通过 GitHub Merge Queue 合并。
...
- PR 经 GitHub Merge Queue 合并到 `main` = Issue 可以进入最终验收/关闭流程。
- `main` push 会触发 deploy；如果 deploy/runtime 失败且不是该 PR 逻辑问题，另开 release/deploy issue 追踪。
...
Drone 通知在 success 和 failure 都发送到 Telegram。通知脚本 `scripts/ci/drone-telegram-notify.py` 会解析负责 Agent 并在群里 @ 对应 bot。

`main` push build 来自 feature/fix/ci 分支通过 GitHub Merge Queue 合并，通知必须尽量溯源到对应功能 Agent 并 @ 对应 bot。`devops` push 仍视为 CI/CD lane，固定归 Eino：

```text
Agent: hermes @ws_ubuntu_hermes_bot
Attribution: commit subject / merged source branch   # main push

Agent: eino @eino_hermes_bot
Attribution: devops owner                            # devops push
```

开发 PR / `main` push 的归因来自任务分支和 commit metadata：
...
3. Commit subject marker [agent]
4. Commit trailer Agent:
5. Git author email
6. unresolved; do not auto-mention an agent
```

如果多个来源不一致，通知优先 @ 可验证来源（PR 分支 / main merge source branch / subject marker）对应的 Agent，并展示 `Attribution mismatch` warning，方便 controller 追责。
