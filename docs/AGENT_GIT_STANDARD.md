...
`main` / `devops` 的 push build 是 release / CI-CD operation，不做功能 Agent 归因推断，固定归 Eino：

```text
Agent: eino @eino_hermes_bot
Attribution: main release owner   # main push
Attribution: devops owner         # devops push
```

开发 PR / 合入 `develop` 的归因仍来自任务分支和 commit metadata：
...
4. Git author email
5. unresolved; do not auto-mention an agent
```
