...
2. **Git author email 是第一归因来源**：即使所有 Agent 通过同一个 GitHub 账号 push，commit 本身必须带有该 Agent 的专用 Git identity。
3. **commit trailer 是第二归因来源**：每个 commit 必须包含 `Issue`、`Agent`、`Human-Owner` trailers。
4. **分支名必须在第二段带可信 Agent 名**：分支格式是 `<type>/<agent-name>/<issue>-<task-desc>`；CI 会硬性校验 `agent-name` 属于当前 Agent 团队。
5. **归因冲突时宁可不 @ 人**：如果 Git author email 与 `Agent:` trailer 不一致，CI/CD 必须标记 attribution mismatch，并避免自动 @ 任何 Agent。
