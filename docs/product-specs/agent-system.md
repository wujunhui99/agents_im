...
6. 单独管理 Agent skills；skill 元数据在 PostgreSQL，skill 文件在 RustFS (S3-compatible) object storage。
...
- Skill 元数据单独管理。
- Skill 文件保存在 RustFS (S3-compatible) object storage。
...
- Skill 文件内容不进入 PostgreSQL；RustFS/S3 上传和读取执行链路不在本 registry 任务范围内。
...
3. Message Service 的 `MessageCreatedHook` 将已持久化消息快照交给 Agent hosting seam；后续异步 IM event consumer 可复用同一契约。
...
写回消息必须持久化为 `message_origin=ai`，并携带 `agent_account_id`、`trigger_server_msg_id`、`agent_run_id` 和 `allow_recursive_trigger=false` 默认语义。普通用户消息为 `message_origin=human`，系统提示/状态消息为 `message_origin=system`。
...
第一阶段 conversation hosting 使用 `agent_conversation_hosting` 表表示某 `conversation_id` 是否由某个 `agent_account_id` 托管，并用 `agent_trigger_idempotency` 记录 `event_id/server_msg_id + agent_account_id` 幂等状态，避免重复事件产生重复回复。
...
- `MessageCreatedHook` 在消息持久化后触发 `ConversationHostingService`，并使用稳定 `message.created:<server_msg_id>` idempotency event id。
