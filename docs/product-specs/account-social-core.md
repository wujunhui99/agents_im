...
- `AddFriend` 创建 `pending` 好友申请，不立即进入普通好友列表。
- 只有申请接收方可以接受或拒绝 pending 申请；接受后创建双向 `accepted` 好友关系。
- 重复添加同一 pending 申请或同一有效好友是幂等成功，返回已存在关系，不创建重复关系。
- 添加自己为好友必须拒绝。
- 目标账号不存在时必须拒绝；好友关系不写入 Account Service 的账号资料字段。
- `DeleteFriend` 使双向关系失效；`ListFriends` 只返回当前 `accepted` 好友；关系查询对从未添加返回 `none`，对 pending/rejected/deleted 返回对应非好友状态。
...
1. 用户发起添加好友。
2. `friends` 写入 pending 申请，等待接收方接受或拒绝。
3. 接收方接受后写入双向 accepted 好友关系，拒绝后不进入普通好友列表。
4. 用户可以删除已接受好友。

验收标准：

- 添加好友后能查询到 pending 好友申请。
- 接收方接受后双方能查询到 accepted 好友关系。
- 接收方拒绝后双方普通好友列表不可见该关系。
- 重复添加同一 pending 申请或有效好友保持幂等成功。
- 添加自己或添加不存在账号必须失败。
- 删除好友后关系失效。
- 好友列表只返回当前 `accepted` 关系；关系查询能区分 `pending`、`accepted`、`rejected`、`deleted` 与 `none`。
