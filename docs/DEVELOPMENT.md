...
服务名和 go main package path 的单一事实源是 `scripts/services.json`(被 `Makefile`、`Dockerfile`、`scripts/deploy-k3s.sh`、`scripts/detect-deploy-changes.py`、`scripts/dev-up.sh` 共用);新增/删除部署服务只改这一处。生产镜像按服务发布为 `<service>:<sha>`，每个后端镜像只含 detector 选中的单个 `/app/service` binary；web 仍是独立镜像。本地各服务的配置模板在 `scripts/dev/etc/*.yaml.tmpl`(数据),`scripts/dev-up.sh` 用环境变量渲染它们;启动顺序和默认端口以 `scripts/dev-up.sh` 为准。下表只作常用端口概览。
