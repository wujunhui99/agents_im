...
- [rest-service-context-boundaries.md](./rest-service-context-boundaries.md)：REST/Gateway focused ServiceContext 边界（**Superseded/历史存档**：顶层 `internal/` 已于 #618 完全退役）
...
- [distributed-tracing-tempo.md](./distributed-tracing-tempo.md)：OpenTelemetry Collector + Grafana Tempo 分布式追踪后端、查询和运维约束
- [llm-observability.md](./llm-observability.md)：AI Hosting LLM 观测、Langfuse sink 边界和 deterministic eval harness
...
- [agent-system-architecture.md](./agent-system-architecture.md)：Agent 账号类型、prompt/tool/skill registry、RustFS skill 文件、MCP 和 Python Executor 第一版架构
