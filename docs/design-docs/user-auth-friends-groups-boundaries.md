...
- 明确 `account_type=0|1|2`（0=管理员，1=用户，2=Agent），其中 `user` 是 account type，不是服务名。
...
- 管理 `account_type=0|1|2`（0=管理员，1=用户，2=Agent）。
...
- 重复添加同一有效好友保持幂等成功；添加自己或添加不存在账号必须失败。
...
6. `GetFriendship` 对有效关系返回 `active`，对已删除关系返回 `deleted`，对从未添加的账号对返回 `none`。
