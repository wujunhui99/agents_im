...
Status: Accepted / Issues #74 and #142 implementation boundary
...
- Make Langfuse the primary intended backend, with live network export opt-in and explicit.
...
- Langfuse export is not a replacement for system-level request tracing, Prometheus metrics, or admin LLM trace/audit APIs.
...
- `LangfuseSink` exports a sanitized batch to `POST /api/public/ingestion` on the configured host using HTTP Basic auth (`LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY`). Non-2xx responses and Langfuse ingestion errors return explicit errors and never report `Exported=true`.
...
LANGFUSE_HOST=https://langfuse.agenticim.xyz
```
...
> **已移除（2026-06-13）**：`internal/agenteval` 是从未被任何代码引用的孤立 harness（重构台账 04-agent §AG-11），随 internal 退役一并删除。原先它包含确定性用例 `ai_hosting.python_go_performance.v1`，规则裁判对模糊回复打不及格、对覆盖执行速度/编译 vs 解释/并发/部署/内存等维度的回答给通过。如需重建评估能力，按 00-decisions D10 落到 `service/agent/rpc/internal/eval`。
...
- Langfuse metadata is redacted by key and value before export. JWTs, bearer values, cookies, passwords, secrets, API keys, and DSNs must not be sent raw.
...
```bash
PATH=/home/ws/go/pkg/mod/golang.org/toolchain@v0.0.1-go1.24.1.linux-amd64/bin:$HOME/go/bin:$PATH GOCACHE=/tmp/go-build-cache go test ./internal/llmobs ./internal/agentim ./internal/config
```
