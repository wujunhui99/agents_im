...
Scope: `gateway-ws`、浏览器 WebSocket、`message-transfer`、Kafka/Redpanda、k3s/Ingress/ConfigMap/Deployment
...
- 阶段二：实时 fanout 链路未启用：`message-transfer` 生产配置仍是 `Dispatcher.Driver: noop`，不会把 `message.accepted` 事件投递到 `gateway-ws` 在线连接。
...
browser /ws?token=[REDACTED]
  ...
  -> gateway-ws internal delivery endpoint
  -> gateway-ws in-memory connection manager
...
http://127.0.0.1:8084/internal/delivery/conversation
        |
        v
gateway-ws PushToConversation / PushToUser
        |
        v
Browser B receives { type: "message_received", data: ... }
```

当前 k3s 生产是单副本 `gateway-ws`，并且 `gateway-ws` 使用 `hostNetwork: true`。因此 `message-transfer` 可以通过 `http://127.0.0.1:8084` 调用同节点共址的 gateway 内部投递 endpoint。后续如果 `gateway-ws` 多副本或跨节点，需要改为 Redis/NATS/Kafka fanout 或按 presence route 投递到对应 gateway instance，不能继续假设 localhost 即目标连接所在进程。
...
`deploy/k8s/etc/gateway-ws.yaml`：
...
`etc/gateway-ws.yaml` 本地开发也应允许 query token，避免本地与生产行为分叉：
...
这会导致 worker 即使消费到 `message.accepted`，也只是“假装成功”，不会调用 `gateway-ws`，所以在线 B 收不到 frame。
...
3. 把 `gateway-ws` 返回的 `delivery.Result` 映射回 `transfer.DispatchResult`。
...
kubectl -n agents-im get deploy gateway-ws message-transfer -o yaml
```

确认点：

- `gateway-ws` image digest/tag 是否为最新 commit 对应镜像。
...
当前 `gateway-ws` 使用 `hostNetwork: true`，生产单副本时 `message-transfer -> http://127.0.0.1:8084` 能命中同节点 gateway。

风险：

- 如果未来 `gateway-ws` 扩多副本，某个用户连接可能在另一个 gateway 进程。
- 如果 `message-transfer` 与 `gateway-ws` 不共址，localhost 会打到错误进程或无服务。
...
`gateway-ws` 的在线连接保存在本进程 `ConnectionManager`。如果通过 Kubernetes Service 随机打到任意 gateway Pod，则可能命中没有该用户连接的 Pod，返回 offline/routed。
...
kubectl -n agents-im rollout status deploy/gateway-ws
kubectl -n agents-im rollout status deploy/message-transfer
kubectl -n agents-im logs deploy/gateway-ws --since=20m
...
```bash
kubectl -n agents-im logs deploy/gateway-ws --since=20m | grep -Ei 'websocket|handshake|origin|unauthorized|connected|disconnected'
```
...
kubectl -n agents-im get deploy gateway-ws message-transfer -o wide
```
...
- `gateway-ws` internal conversation delivery endpoint。
