...
适用场景：修改鉴权、secret、日志/trace、Agent 工具、外部访问、部署凭据或敏感数据处理时读取本文。
...
- Tempo trace 数据可能暴露内部拓扑、路由、trace id、错误和延迟信息；Tempo 服务必须保持 internal-only，不得新增公网 Ingress。Grafana 作为查询入口，必须保留登录认证。
