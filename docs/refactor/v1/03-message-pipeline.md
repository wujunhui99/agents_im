...
| A3（✅ #492） | gateway-ws 4 个 ws command（send_message / pull_messages / mark_conversation_read / get_conversation_seqs）改调 msg-rpc gRPC，删 in-process 接线；随本 PR 改名 `service/msggateway` | ACK 语义不变（msg-rpc 仍同步写 PG 返回 seq），客户端零感知。**落地**：ws server 迁 `service/msggateway/internal/ws`、推送契约迁 `common/share/gateway`（msgtransfer 共用）；send 后 gateway 不再本地 fanout（message_received 含 sender seq 回填均由 msgtransfer 下发）；ws 集成测试迁包内 fakeBackend 接线；single-machine e2e 去 ws leg；删 `internal/servicecontext/gateway` |
