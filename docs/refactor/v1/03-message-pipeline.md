...
## 9. 分阶段实施（替换原版的 Phase 0~4）

### Phase 0：清理 outbox 残骸 + 文档（1~2 周）
- 删 `internal/outboxpublisher/` 整个包；
- 删 `internal/repository/message_outbox_repository.go`、`internal/repository/postgres_outbox.go`；
- migration 加 `017_drop_message_outbox.sql`（保留表数据备份，DROP 在下个版本）；
- 删 `etc/msgtransfer.yaml` 的 `Consumer.Driver: outbox` 选项；
- 删 `internal/transfer/outbox_consumer.go`；
- 更新 ARCHITECTURE.md "Message Pipeline" 一节。

### Phase 1：msg-rpc 改为只 publish Kafka（2~3 周）
- 新建 topic `msg.toTransfer.v1`；
- msg-rpc.SendMessage 改为只 `producer.Publish`，不再写 PG；
- ACK 不再返回 seq（API 契约变更，前端配合）；
- 兼容期：保留旧路径（开关 feature flag `MSG_DIRECT_PG_WRITE=true`）；
- 灰度切换。

### Phase 2：msgtransfer 实现 batcher + Redis Malloc（3~4 周）
- 引入 `seqConversationCache`（Redis）；
- 引入 `msgCache`（Redis bucket）；
- 实现 batcher（size=500/worker=50/interval=100ms）；
- 实现 `BatchInsertChat2Cache`；
- toPostgres consumer 批量写 PG；
- 删除 msg-rpc 内的 seq 生成代码（迁移到 transfer）。

### Phase 3：拆 push 进程（2~3 周）
- 新建 `service/push/`（goctl main 即入口），Makefile 注册 push 服务；
- 实现 toPush consumer + online 广播；
- 实现 toOfflinePush producer/consumer + FCM adapter（or noop）；
- msgtransfer dispatcher 改为 produce toPush topic（不再调 gateway HTTP）；
- 删除 `internal/transfer/gateway_http_dispatcher.go`、`internal/transfer/gateway/`。

### Phase 4：gateway 改为纯连接层（2~3 周）
- msggateway 删 import `internal/logic`、`internal/repository`；
- msggateway 改为只持有 msg-rpc client；
- msggateway 注册 gRPC GatewayService 给 push 调；
- 调通 push → gateway 广播链路。

### Phase 5：Redis 高可用 + 监控（与 Phase 1~4 并行）
...
| 风险                                        | 应对                                             |
|---------------------------------------------|--------------------------------------------------|
| Phase 1 ACK 语义变化，前端报错              | feature flag 灰度；前端在 SDK 用 client_msg_id 占位渲染（OpenIM SDK 已有模式可抄）|
| Kafka 不可用导致 SendMessage 失败           | 生产前必须 Redpanda 3-broker 集群 + acks=all；监控 producer error rate |
| Redis 单点导致 seq 不可用                   | Phase 5 上 Sentinel 是硬前置                      |
...
| 现有 outbox 数据如何处理                    | Phase 0 不立即 DROP 表；保留 90 天观察           |

---

## 12. 待确认问题

1. **Redpanda producer acks=all 是否会显著拖慢 SendMessage？** 需要在 Phase 1 实测，目标 p99 < 50ms。
...
原版（已被本文件覆盖）保留了 outbox + 推了"msg-rpc → outbox → transfer → push"链路。本版基于 OpenIM 代码精读后修正为：

- ❌ outbox 弃用；
- ❌ msg-rpc 不再写 PG；
- ✅ seq 在 transfer 用 Redis Malloc；
- ✅ Redis 升级为热消息缓存（不仅 presence）；
- ✅ push 用广播路由，不查 presence 走 per-instance；
- ✅ 二段式离线推送（toPush → 失败再 toOfflinePush）；
- ✅ msggateway 砍掉所有业务逻辑依赖，只剩连接层。

原版 §3 OpenIM 架构概述、§5 与现代码映射、§7 设计取舍记录 → 本版 §1、§3.4、§2.4 替代。
