...
| Agent 触发                       | 无（OpenIM 不带 Agent）               | `agent.trigger.v1` 全量 produce 零分支，全部判定（含防递归）在 agent-rpc 消费端（§10 / D15）；toPush 按 recv_id 类型位过滤 agent 收件人（D16） |
...
| A3（✅ #492） | gateway-ws 4 个 ws command（send_message / pull_messages / mark_conversation_read / get_conversation_seqs）改调 msg-rpc gRPC，删 in-process 接线；随本 PR 改名 `service/msggateway` | ACK 语义不变（msg-rpc 仍同步写 PG 返回 seq），客户端零感知。**落地**：ws server 迁 `service/msggateway/internal/ws`、推送契约迁 `pkg/gateway`（msgtransfer 共用）；send 后 gateway 不再本地 fanout（message_received 含 sender seq 回填均由 msgtransfer 下发）；ws 集成测试迁包内 fakeBackend 接线；single-machine e2e 去 ws leg；删 `internal/servicecontext/gateway` |
| A4 | ✅(部分,#341) `internal/servicecontext/message` 重定位出 internal→`service/msg/rpc/internal/aihosting`；**待** 删 `internal/logic` 消息域剩余（messagelogic 等）+ `internal/repository` 消息部分 | keystone 解锁；删空顶层 `internal/` 的前置；aihosting 的 AI 托管 runtime 仍消费 `internal/logic`/`repository`，剩余删除需与触发点迁移（B1，agent AG-2/AG-3）协同 |
...
| B1（✅ #474 落地） | msgtransfer 新消费链路（additive、dormant）：batcher（500/50/100ms，conv hash 分片，§5.1）+ Redis seq Malloc + msgCache + toPostgres consumer 批量写 PG，消费 `msg.toTransfer.v1`（此时无人生产）；categorize 内含 Agent trigger（produce `agent.trigger.v1`，§10 / 文档 04 §4.2）；随本 PR 改名 `service/msgtransfer` | 复刻 additive 模式：先建下游，零流量零风险。**落地偏差**：(1) batcher 用 poll-batch barrier（按 conv 分组并行处理一个拉取批、全部完成才 commit offsets，franz-go 手动提交）替代 OpenIM 自由流 batcher——单实例低流量等价，at-least-once 重放靠 Redis dedup 收敛（链路集成测试覆盖）；(2) `agent.trigger.v1` 对每条 storage 消息无条件 produce，hosting/recursion 判定留在消费方（B2 落 msg-rpc runtime，与现 fireMessageCreatedHook 语义对齐）——后续口径已定稿 D15：前移到 transfer 的只有零依赖轻判，hosting 终判永远留 agent-rpc 消费端；(3) **新链路 push fanout 始终含 sender**（ACK 无 seq 后 sender 靠自己的 message_received 回填占位，§4.2）；(4) toPush 经 `KafkaPushConsumer` 适配现有 transfer.Worker + gateway HTTP dispatcher（C2 才拆 push 服务）；(5) seq 计数器 Redis miss 时从 PG max(messages.seq) 播种，(conversation_id,seq) 唯一冲突 = seq 回退告警（persist consumer 日志含恢复指引），不静默吞 |
| B2（✅ #479 落地） | msg-rpc SendMessage 加 feature flag `MSG_DIRECT_KAFKA`：MsgToMQ 只发 Kafka、不写 PG、ACK 去 seq；前端 SDK 以 client_msg_id 占位渲染（§4.2 / §7.2） | **唯一契约破坏点**，集中在一个 PR（后端+前端，PR #483）；flag 切回 = 秒级回滚。**落地偏差**：(1) flag 读 yaml `Kafka.Enabled`，env `MSG_DIRECT_KAFKA` 优先覆盖（秒级回滚开关）；(2) Kafka 模式下 AI 写回经 `AgentResponseSender` 注入本进程 SendMessage（防 PG/Redis 双 seq 分裂），`agent.trigger.v1` consumer 在 msg-rpc 内回流 AgentHook（04 落地前的过渡）；(3) 前端 authoritativeSeq 区分 seq=0 占位与权威 seq，防误判已读 |
| B3a（✅ #484 切流） | prod 开 `MSG_DIRECT_KAFKA`（`deploy/k8s/etc/msg-rpc.yaml` Kafka.Enabled=true）→ 冒烟 + 观察（consumer lag / seq 连续性 / PG 异步写延迟） | 回滚 = deployment 加 env `MSG_DIRECT_KAFKA=false` 或 revert |
| B3b（✅ #494） | 观察稳定后退役旧路径：删 `internal/outboxpublisher/`、`internal/transfer/outbox_consumer.go`、`internal/repository/{message_outbox_repository,postgres_outbox}.go`、msg-rpc 内 PG 写与 seq 分配代码；更新 ARCHITECTURE.md "Message Pipeline" | **落地**：Kafka 成唯一写链路（msg-rpc / msgtransfer 缺 brokers 启动失败，MSG_DIRECT_KAFKA=false 显式拒绝）；msgtransfer legacy worker 整体退役（chain 自带 pushWorker，Consumer 配置块从 yaml 移除）；monolith messagelogic/repo 的 outbox 写一并删除（A3 后无生产调用方）；本地 dev 补 redpanda（docker-compose + dev-up，KAFKA_BROKERS 默认 localhost:19092）；`message_outbox` 表**不 DROP**，保留 90 天观察（§11）后另行 migration。前置 A3 已落地（#492）；A4（删 internal 消息域剩余）不阻塞本步——outbox 写入点已随本 PR 摘除 |
...
> 2026-06-12 定稿（00-decisions **D15**，#499 / #501 两次修正）：toAgent **全量 produce、
> transfer 零分支**，全部触发判定（含防递归）收敛 agent-rpc 终判；**D16** 账号类型编进
> account_id（Sonyflake 变体 5 位类型位）后，toPush 按 recv_id 类型位过滤 agent 收件人
> （零查询）。早期表述作废：①"categorize 查 hosting 配置 / 解析 @"——主链引入 agent 域同步
> 依赖；②"事件携带 receiver_account_type / trigger_kind"——已被 D16 取代（类型在 ID 里）。

Agent 触发链路（文档 04 §4.2）：

```
msg-rpc SendMessage → msg.toTransfer.v1（事件 schema 不为触发判定加字段；
  账号类型已编码在 account_id 类型位，D16）
        ↓
msgtransfer categorize（dedup + seq 之后，同批 produce 三路）：
  ├─ msg.toPostgres.v1：全量（归档）
  ├─ msg.toPush.v1：唯一过滤 = recv_id / 群成员 ID 类型位 = agent → 不进 fanout
  │   （D16 零查询；显式语义：agent 账号无连接面、不接收推送，C3 离线推送同此）
  └─ agent.trigger.v1：全量 produce、零分支（key=conversation_id，事件带 seq）
      ——每条人类消息本质都要判一次托管，判定次数是不变量；全量 produce 把这次
      判定放到异步消费端 + 数据 owner，热路径零负担
        ↓
agent-rpc（consumer group agent-trigger，Kafka 上零生产面）终判三步：
  1. origin=ai 且未标 allow_recursive_trigger → 丢弃（防递归闸门，事件字段零查询）
  2. receiver / 群成员 ID 类型位 = agent（D16 零查询）→ agent 收信触发
  3. conversation ∈ conversation_ai_hosting（自己域内本地 model）→ 托管触发
...
agent-rpc 终判第 1 步拦下 → 递归终止；trigger event 带 source_agent_run_id 兜底幂等。
```

不拆双 topic（agent 收信 / 会话托管两类场景共用 `agent.trigger.v1`）：消费者同一个、处理
代价同源、顺序需求相同；同会话可能同时命中两类，拆开丢失 conversation_id 分区键的会话内
有序。判定移到消费端新增的只有运输成本（每条消息一份 Kafka 事件写读）；量级需要时在
transfer 加托管会话集合的 Redis 缓存预过滤（只改发多发少，不动结构）。

现状（B1/B2 过渡态）：trigger 已全量 produce、判定在消费端（msg-rpc 回流 AgentHook）。
迁移按 D15 四步：D16 账号 ID 切换+数据清零 → transfer 加 toPush 类型过滤（toAgent 零改动）
→ agent-rpc 新 group 消费（三步终判）→ 删 msg-rpc 回流接线（解锁 A4）。事件 schema 零变更。

接口先行落地（#503）：步骤②③的**代码面**已就位——transfer toPush 过滤实现于
`service/msgtransfer/internal/chain`（挂 `Kafka.TypedAccountIds` 开关，**默认关**，
步骤①数据清零前严禁开启：存量旧版 snowflake ID 的类型位是噪声）；`service/agent`
（扁平 main，未部署）以新 group `agent-trigger` 消费并承载三步终判，runtime/写回/
hosting 均为显式 mock driver（零副作用）。类型位唯一解析入口 = `pkg/idgen`
（`AccountIDType` / `IsAgentAccountID`，D16 解析纪律）。步骤①（ID 切换+迁移+清零）
与步骤④未做，真实 AI 回复仍由 msg-rpc 回流 consumer 产生。
...
5. ~~**Agent trigger 是否也用 transfer 内 inline 判断**~~：已定稿（D15 / §10）——单 topic `agent.trigger.v1`，transfer 零依赖轻判 + agent-rpc 终判，不做 in-process、不在 transfer 查库。
