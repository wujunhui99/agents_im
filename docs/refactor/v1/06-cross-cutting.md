...
> ✅ 已修复（#381，B 方案）：拆出 `service/media/{api,rpc}` 独立服务，ingress `/media` → media-api:8089（4 条路由 live）。Stage 4 已删孤儿 `internal/handler/media/` 与 message-api #380 临时头像路由+objectStore。
> 仍保留：message 媒体 owner 校验（`internal/logic/medialogic.go` `ValidateMessageMedia`）；usersvc legacy MediaLogic plumbing 待后续随 monolith 死代码一并清理。
