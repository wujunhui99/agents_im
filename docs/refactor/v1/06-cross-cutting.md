...
> ✅ 下载授权（#532，EPIC #527 §4，**BFF 聚合**）：`/media/:media_id/download-url` 加 `?msg_id`。鉴权编排在 **media-api(BFF)**：`media-rpc.GetMedia` 判 uploader 快速放行 → 否则 `msg-rpc.GetMessageRef` 链路校验（消息引用的 media_id 必须等于入参 media_id，防越权）+ 私聊单向好友（`friends.GetFriendship` 只看 requester→peer）/ 群成员（新增 `groups.IsMember`）校验 → 通过后 `media-rpc.GetDownloadURL` 纯签发。**media-rpc 不再依赖 `internal/repository`、也不发起 rpc→rpc 调用（保持叶子）**；旧 `AttachmentAccess.UserCanAccessMedia` 与管理员兜底退役，下载纯按当前关系判定。为何放 BFF：避免 `media-rpc ↔ msg-rpc` 形成 rpc 调用环（见 AGENTS.md「微服务分层」）。
> 仍保留：message 媒体 owner 校验（`internal/logic/medialogic.go` `ValidateMessageMedia`）与 media-rpc 复用的 `internal/logic.MediaLogic`。

### XC-13 ⚠️ RustFS 与 PG `media_objects` 一致性
当前流程：客户端预签名上传 RustFS → 客户端调 complete_upload → user-api 校验 → 写 `media_objects` 表。如果客户端上传成功但没回调 complete_upload，RustFS 里会留孤儿对象。

> 修复方向：RustFS event notification → cleanup worker；或者每天 reconcile job 扫 orphan。

### XC-14 ⚠️ objectstorage 抽象 OK
`internal/objectstorage/{factory.go, memory.go, s3.go, store.go}` 抽象合理。Memory implementation 用于测试。继续保留。
