...
> ✅ 下载授权 media 编排（#532，EPIC #527 §4）：`GetDownloadURL` 入参加 `msg_id`，非 uploader 走 `MsgRpc.GetMessageRef` 链路校验（消息引用的 media_id 必须等于入参 media_id，防越权）+ 私聊单向好友（`friends.GetFriendship` 只看 requester→peer）/ 群成员（新增 `groups.IsMember`）校验。media-rpc 注入 msg/friends/groups 客户端，**删除** `AttachmentAccess.UserCanAccessMedia` 对 `internal/repository` 的反向依赖（连同管理员兜底退役，下载纯按当前关系判定）。
