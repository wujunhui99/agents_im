...
7. **AU 收敛**（D12/D13）：`internal/auth/{logic,token}` 搬进 `service/auth/rpc/internal/`；`repository` 改 goctl model 落 `service/auth/rpc/internal/model/`（废 repository）；**删 `useradapter`/`mailadapter`**——建 account、发信编排上移 auth-api（rpc 间不互调）。
