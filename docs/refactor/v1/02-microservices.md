...
> 修复：写一行规则。建议：**RPC 间直接互调用一律禁止**，需要时显式建立 `service/<domain>/rpc/internal/<other>adapter/`；API 层可以自由 import 任何 RPC client。
...
| AU-2 | `internal/auth/repository/credential_repo` 是 PostgreSQL 实现；service/auth/rpc 通过 svc 依赖它 | rpc 内 repository 没搬进 service/auth/rpc/internal/repository |
| AU-3 | `useradapter` 在 internal/auth 而不是 service/auth/rpc/internal/useradapter | RPC 间通过 adapter 调用 user-rpc 的耦合面没归位 |
...
1. 全部 `internal/auth/*` 搬进 `service/auth/rpc/internal/`：
   ...
   service/auth/rpc/internal/
     ...
     adapter/user.go   # user-rpc 客户端
     adapter/mail.go   # mail-rpc 客户端
     logic/            # 仅入口逻辑
   ```
...
7. **AU 收敛**：把 `internal/auth/{logic,repository,token,useradapter,mailadapter}` 全部搬进 service/auth/rpc/internal/。
8. **CP-3 文档化跨域规则**：在 AGENTS.md 加一行"RPC 间禁互调，API 自由调 RPC"。
