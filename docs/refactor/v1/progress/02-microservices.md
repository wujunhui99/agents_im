...
| friends | goctl+BFF（退役 `core`）| `service/friends/rpc/internal/logic` | `service/friends/rpc/internal/model`（goctl；friendships 代理 PK 迁移 018）| #426 / #606 | rpc 不返回 `Friendship.friend`，BFF 聚合 user-rpc 补全。修正 outgoing 列表 friend 错填请求者→指 friend_id。**#606**：+`EnsureFriendship` RPC（双向同事务幂等 upsert，保留 created_at，无待批流程；`friendships_model.EnsureAccepted`），供 user-rpc 默认助手开通 + agent-rpc `agent.create` 建好友；**彻底删 internal `EnsureAcceptedFriendship`**（接口+memory/postgres 两实现），friends 域不再喂 monolith 好友写。|
...
| msg | goctl+BFF（keystone 域，07 §3.1 接口重设计；分 2 PR）| `service/msg/rpc/internal/logic`（4 RPC 实现，行为对齐旧；新增 6+ RPC stub Unimplemented）| `messages`/`conversation_threads`/`user_conversation_states`/`message_outbox` goctl model（事务编排在 Logic；迁移 019 给 UCS 加代理 PK）| #457（PR1 rpc / PR2 api）| **PR1 rpc（additive，msg-rpc:9098，不动 dormant rpcgen/message）**：跨域 keystone 例外——SendMessage inline 鉴权（群成员=internal GroupsLogic；媒体附件校验 #533 已切 media-rpc ValidateMessageMedia）群成员暂依赖 internal；outbox payload 与 message-transfer 消费契约逐字节一致。**PR2 msg-api（additive，纯 BFF:8090，未切流）**：4 消息路由 .api-first→goctl，BFF over gRPC 调 msg-rpc（msgclient + rpcerror.FromStatus）；svc 无 UserRPC。**#463（=03 §9 A1+A2 合并）已切流退休 message-api/message-rpc**：勘探确认 web 走 REST POST /messages（WS 只收事件），AI 托管触发钩子在 message-api 进程内 → 触发钩子+hosting runtime 整套迁入 msg-rpc（svc.newConversationAIHostingRuntime 移植 message-api main 接线，SendMessage persist 后 fireMessageCreatedHook，语义对齐含 dedup；config 加 DeepSeek/LLMObservability/PythonExecutor，deployment 加 python-executor-controller SA）；msg.proto +Get/UpdateConversationAIHosting（keystone 例外注明待 B1 迁 msgtransfer/agent 域）；admin-rpc +CreateFeedback（feedback 数据层 owner）；msg-api 7 路由 BFF（+ai-hosting×2→msg-rpc、/api/feedback→admin-rpc，裸 /feedback prod 不可达随退役丢弃）；ingress 三前缀→msg-api:8090；删 service/message-api、internal/rpcgen/message、internal/handler、internal/logic/message；部署/CI/verify 全链清理（Dockerfile/Makefile/drone-build/deploy-k3s/detect-deploy/kustomization/dev-up/test-deploy-k3s/verify-*/vite proxy）；JWT-sender 契约测试迁 msg-api 单测（fake msgclient）。**A3 ✅ #492**（gateway-ws→msggateway，4 ws command 走 msg-rpc gRPC，ws server 迁 service/msggateway/internal/ws、契约迁 pkg/gateway）。**A4（进行中）**：`servicecontext/message` 已 **#341** 重定位出 internal → `service/msg/rpc/internal/aihosting`（脱 god-package，由 service/<domain>/<kind> 通用 deploy 规则路由 msg-rpc，删 detect-deploy internal 前缀特例）。剩余 `internal/logic` 消息域（messagelogic）+ `internal/repository` 消息部分仍是 keystone——aihosting 的 AI runtime 仍消费，待 agent 域迁移（04-agent AG-2/AG-3）删 `newConversationAIHostingRuntime` 整套接线时一并清（与 B1 协同）。写路径 Kafka 化进行中：**B0 ✅ #470**（Redpanda 单 broker 入 deploy/k8s/middleware，acks=all 实测 200msg/s p99=4ms；单节点偏差记 03 §9 B0 行；顺带修 Drone PR verification 全跳过 #472）；**B1 ✅ #474**（service/message-transfer 改名 service/msgtransfer + main 文件改 msgtransfer.go；franz-go acks=all 幂等 producer/consumer 封装入 pkg/messaging（topics + message.submitted 事件）；service/msgtransfer/internal/chain 新消费链：toTransfer poll-batch barrier + Redis seq Malloc（PG 播种）/dedup 7d/msgCache bucket-100 → produce toPostgres/toPush/agent.trigger → persist consumer 批量事务写 PG（手写 model，worker 无 CRUD 面故未走 goctl 生成）→ KafkaPushConsumer 适配现有 Worker+gateway dispatcher；prod 配置 Kafka.Enabled=true dormant；CI verification 加 redpanda/redis services 跑链路集成测试；偏差详单见 03 §9 B1 行；合并后需手工 kubectl delete 旧 deployment/service message-transfer）。**B2 ✅ #479 / B3a ✅ #484 / B3b ✅ #494**（B3b：Kafka 单轨——删 msg-rpc 非 Kafka 写分支+outbox model、msgtransfer legacy outbox worker、internal/outboxpublisher 与 repository outbox 残留；dev 补 redpanda；message_outbox 表留 90 天观察）。**C ✅ #342（拆 push 完成）**：新建 `service/push`——消费 `msg.toPush.v1` → 经 k8s headless Service DNS `msggateway-headless:9100` 广播 `GatewayService.BatchPushOneMsg` 到所有 gateway 实例（**无 etcd**，go-zero dns:/// 负载均衡只命中单实例故自维护一 IP 一连接的广播池）→ 汇总 per-user 投递 → 漏投 user 二段式 produce `msg.toOfflinePush.v1`；自身 offline consumer 调 `AuditOfflinePusher`（首版 audit-only，§6.3 允许）。msggateway 加下行推送 gRPC server（`service/msggateway/gateway.proto`→`gatewaypb`，listen 9100）。msgtransfer 只保留 produce toPush，**整包删 `service/msgtransfer/internal/transfer/`**（worker+HTTP/local gateway dispatcher+delivery-attempt recorder 随 push 退役，msgtransfer 不再 import internal/repository）。新 topic/group：`msg.toOfflinePush.v1` / `push-online` / `push-offline`（D5）。偏差：result 聚合=任一 gateway delivered 即 delivered、任一 RPC 失败整批重试（不误判离线，at-least-once）；离线首版无厂商通道（`agents_im_push_offline_total` 指标+日志）；push 纯调度器无 DB，原 delivery_attempts PG 记录暂以指标替代。全链已串 Dockerfile/Makefile/drone-build/deploy-k3s/detect-deploy/deployments+services（含 msggateway-headless）/kustomization/dev-up/verify；push observability 用 8091（避 hostNetwork 下与 agent-api:8086 撞）。**待**：Redis 读路径/RevokeMessage 等（D，原 07 Phase2-3）。**#555（gate #550 第3步）**：AI 托管 agent-create 工具路径（`AgentAssemblyLogic.CreateAgentFromTool`）的账号读写脱 internal——新增 `service/msg/rpc/internal/userrpc` **Composite**（实现 `repository.Repository`：账号 `Create`/`GetByID`/`ExistsByIdentifier` over user-rpc、其余账号写 fail-loud；好友方法委托内部 postgres repo，`friendships` 表无 avatar 暂留），svc `messageContext.AccountRepo = Composite`，config/etc/k8s/dev-up +`UserRPC`。群成员鉴权本就 avatar-free（`NewGroupsLogic(repo, nil)`，lookup==nil 不 hydrate）。原子性：account 改 user-rpc 后 `TransactRepository` 类型断言失败→落非事务路径（跨服务尽力而为，参照 AdminBootstrap；孤儿 agent 账号风险极低）。无 .proto/.api 变更。|
| agent | goctl（数据层分片退役，AG-6/D13 多 PR；编排已 #340 迁入 agent-rpc）| `service/agent/rpc/internal/convhosting`（业务规则 + Store）；`service/agent/rpc/internal/registry`（注册表只读 Store）| `service/agent/rpc/internal/model` goctl `conversation_ai_hosting_settings`（迁移 024）+ `agent_prompts/tools/skills`、`mcp_servers`、3 binding（迁移 025 复合 PK→代理 `id`+`UNIQUE` 保 `ON CONFLICT`；custom `FindByAgentId`）| #603（AG-6 ①）/ #605（AG-6 ②）/ #606（AG-6 ③）| **AG-6 ① 仅 `conversation_ai_hosting_settings`**：勘探确认仅 agent-rpc 消费（msg-api 经 gRPC 调，不 import internal）→ **整片删 internal**（非 shim）：`internal/repository/conversation_ai_hosting_*`+`postgres_conversation_ai_hosting.go`+`internal/logic/aihostinglogic*` 与 `postgres_common` 入口已删；`ConversationAIHostingLogic`（含 `AgentAccountExistenceChecker`、`singleConversationParticipants`）搬入 `convhosting`，背靠 Store（model-backed prod / memory fixture）；重接 4 处消费——trigger `hosting.Store`、gRPC CRUD `logic`、orchestrator request builder + `ConversationHostingService`。**AG-6 ②（#605，registry 读路径 leaf）**：7 张注册表（prompts/tools/skills/mcp + 3 binding）goctl model 落 `rpc/internal/model`（迁移 025 给 3 binding 加代理 `id`、复合键降 `UNIQUE`）；新增 `registry.Store`（只读，bigint 列 ↔ string ID 转换，沿用 #013 bigint keystone 约定，`custom FindByAgentId`）；**重接 2 处 agent-rpc 进程内只读消费**——orchestrator 请求构建器 + runtime tool 解析器（`AgentRegistryReader`/`tools.Registry`），脱 `internal/repository`。CI：drone-postgres-integration 新增 `./service/agent/rpc/...`（覆盖 convhosting+registry 的 bigint=text 边界）。**AG-6 ③（#606，写/装配/BFF）**：`agents` 表 goctl model 落 `rpc/internal/model`（单 PK，无迁移）+ 注册表写补全（returning-insert/UpsertByName/ListActive/BindOne 幂等/ReplaceForAgent 单表原子）；`registry.Store` 从只读扩成 **读+写**（string↔int64 + unique→AlreadyExists/FK→NotFound/check→InvalidArgument 错误映射）；agent 业务逻辑从 `internal/logic` 迁入 `rpc/internal/agentlogic`（`AgentStore` 接口 + goctl `PgAgentStore` 实现 / `MemoryAgentStore` fixture；`AgentLogic`/`AgentRegistryLogic`/`AgentAssemblyLogic`/`DefaultAssistantProvisioner`，对外 string ID，注入 `AccountPort`=user-rpc / `FriendPort`=friends-rpc）；agent.proto +CRUD/定义/`EnsureDefaultAssistant` gRPC 面（消息用 `AgentEntity`，避与 service 同名 panic）；**agent-api 转纯 BFF**（只持 `AuthRuntime`+`AgentRPC`，删 in-process logic/DB/PythonExecutor/UserRPC，缺 AgentRPC 配置 fail-loud）；`agent.create` saga 跨 user/agent/friends 转尽力而为非事务（#555 先例）；`EnsureDefaultAssistant` 只动 agent 域自有数据、不回调 user-rpc 故无环。**删 internal**：`internal/logic/{agentlogic,agent_definition,agent_registry,default_assistant}`(+_test)、`internal/repository/{agent_repository,postgres_agent,agent_memory,agent_registry_repository,agent_registry_memory,postgres_agent_registry}` 及 `EnsureAcceptedFriendship`（接口+两实现+孤儿 helper）；`UserLogic` 去 `WithDefaultAssistantProvisioner`。新单测：agentlogic CRUD/registry 生命周期/assembly happy-path（registry.MemoryStore + MemoryAgentStore fixture）。启动序：user-rpc 启动即 Backfill 调 agent-rpc/friends-rpc，dev-up 把二者排在 user-rpc 前。**AG-6 ④ hosting（#670，从 #616 拆出）**：`agent_conversation_hosting` + `agent_trigger_idempotency`（均 agent 域自有、纯 text PK、无 bigint keystone）goctl model 落 `rpc/internal/model`（custom `Upsert` / `TryStart` TTL 抢占事务 / `Finish` 终态推进）；新增 `service/agent/rpc/internal/aghosting` Store（domain 类型 + 校验 + model-backed `ModelStore` / `MemoryStore` fixture），逐字节复刻旧 `postgres_agent_hosting.go` 语义（failed 总可抢、running 仅超 TTL 可抢、succeeded 不可抢；Finish 只推进 running）；重接 3 处消费——orchestrator `ConversationHostingService.Repository`、agent-rpc `svc`、`aihosting.ServiceContext.AgentHostingRepo`；**删 internal**：`internal/repository/{agent_hosting_repository,agent_hosting_memory,postgres_agent_hosting}`(+两 _test) 与 `postgres_common.NewAgentConversationHostingRepositoryForStorage` 工厂。**剩余 AG-6 ⑤…**：audit（`agent_runs/...`）、MessageLogic 等 message keystone 不动。**不关闭 #344**（删整个 internal 仍依赖剩余 message keystone）。|
| user | goctl+BFF（被依赖最多域）| `service/user/rpc/internal/logic` | `accounts`/`profiles` goctl model（事务编排在 Logic，model 仅 `WithSession`+`Transact`+单一原语；regen 顺带修旧 gen 的 MySQL 方言）| #452 / #606 | ②头像校验 **#533 已切 media-rpc**：`svc.AvatarValidator` 接口由 media-rpc 客户端适配器实现（ValidateAvatarMedia RPC），不再读 `internal/mediavalidate`。**#606**：①默认助手开通改跨域编排——新增 `svc.defaultAssistantProvisioner`（账号经本地 goctl `assistantAccountRepo` 幂等 ensure + legacy `agent_father`→`agent_creator` 改名、agent 域装配经 `agent-rpc.EnsureDefaultAssistant`、好友经 `friends-rpc.EnsureFriendship`），`internal/logic.DefaultAssistantProvisioner` 已删；config +`AgentRPC`/`FriendsRPC`。⚠️ user-rpc `NewServiceContext` 启动即 `Backfill` 调 agent-rpc/friends-rpc，dev-up/k8s 须保证二者先就绪。monolith 消费者（同包 groups logic）仍用 `internal/logic.UserLogic`（已去助手 provisioner 接线），`internal/logic/userlogic.go`+`postgres_user_friends.go` account/profile 部分+`schema_v2_enums.go` 暂保留。logic 依 model 接口 + fake model 单测。user-api 仍纯 BFF（user 是被 hydrate 的源，无跨域聚合）。|
