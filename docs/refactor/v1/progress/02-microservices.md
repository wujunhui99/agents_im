...
| msg-rpc | 部分 | 🚧 #457 PR1 + #463：goctl 数据层；群成员/媒体鉴权 + **AI 托管 runtime/触发钩子**（#463 随 message-api 退役迁入）keystone 暂留 |
...
| auth | lift-and-shift 脱顶层 internal（数据层未 goctl 化）| `service/auth/core/{logic,repository,mailadapter,useradapter,model}` + `service/auth/core/adminbootstrap`（本 PR 由 `internal/auth`+`internal/adminbootstrap` 平移；放 core 而非 rpc/internal 因 e2e single-machine 需跨包 import）| credentials/email_verification 仍是 `core/repository` 的手写 repository（非 goctl）；**#466 起 `rpc/internal/model` 另有 goctl `auth_credentials` model**（EnsureTestCredential 用）| #435 / #466 / 本 PR | 活跃会话 Postgres→Redis 按 (user,device)；共享 `DeviceAuth` 挂 4 个 `jwt:Auth` api。go-zero 坑：`jwt:Auth` 丢注册声明→token 镜像 `user_id`/`session_id`。5 处 inline 校验迁 Redis。`active_sessions` 表+方法成死代码待清。**本 PR**：`internal/auth/*`+`internal/adminbootstrap`→`service/auth/core/*`（消费方仅 auth-rpc + e2e single-machine），顶层 internal 不再有 auth 代码。auth 非 message-keystone 消费（servicecontext/message 不 import auth），平移无需等 message 迁移；但 `core/useradapter`+`adminbootstrap` 仍过渡 import `internal/logic`/`internal/repository`（注册建 user），故完整 goctl+BFF（AuthLogic→rpc、删 useradapter/走 user-rpc）改由 **user 域迁移** 解锁后做。#466 新增跨域鉴权读例外 `model.AccountsGuard`（accounts.account_type，参照 media #433）+ `logic/password_rules.go` 本地 bcrypt（与 core bcrypt-v1 契约一致），auth 域 goctl 化时合并。|
