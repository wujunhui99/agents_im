...
| gateway-ws/transfer | 是 | ❌ keystone；message-api/message-rpc 已退役（#463=A1+A2 合并）；后续按 03 §9：A3 gateway 切 msg-rpc gRPC → A4 删 internal 消息域（注意 AI 托管 runtime 仍消费 servicecontext/message，与 B1 协同）|
...
| auth | 特性改造（非数据层退役）| `internal/auth/logic`（仍 keystone）| 未迁（credentials/email_verification 仍 internal）；**#466 起 `rpc/internal/model` 已有 goctl `auth_credentials` model**（EnsureTestCredential 用，新代码禁 internal）| #435 / #466 | 活跃会话 Postgres→Redis 按 (user,device)；共享 `DeviceAuth` 挂 4 个 `jwt:Auth` api。go-zero 坑：`jwt:Auth` 丢注册声明→token 镜像 `user_id`/`session_id`。5 处 inline 校验迁 Redis。`active_sessions` 表+方法成死代码待清。credentials/email_verification 整体 goctl+BFF（AuthLogic 迁入 rpc、退役 internal/auth）待独立 PR（keystone-blocked）。#466 新增跨域鉴权读例外 `model.AccountsGuard`（accounts.account_type，参照 media #433）+ `logic/password_rules.go` 本地 bcrypt（与 internal bcrypt-v1 契约一致），auth 域整体迁移时合并。|
