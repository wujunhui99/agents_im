...
| msg-api | 否（纯 BFF）| 🚧 #457 PR2：4 消息路由 BFF over gRPC（additive:8090，未切流）；feedback/ai-hosting 暂留 message-api |
| message-api/gateway-ws/transfer | 是 | ❌ keystone；后续按 03 §9 阶段 A~E 重排版（#462）推进：A1 归位 ai-hosting/feedback（#463）→ A2 切流退休 message-api → A3/A4 gateway 切 gRPC + 删 internal 消息域 |
...
| msg | goctl+BFF（keystone 域，07 §3.1 接口重设计；分 2 PR）| `service/msg/rpc/internal/logic`（4 RPC 实现，行为对齐旧；新增 6+ RPC stub Unimplemented）| `messages`/`conversation_threads`/`user_conversation_states`/`message_outbox` goctl model（事务编排在 Logic；迁移 019 给 UCS 加代理 PK）| #457（PR1 rpc / PR2 api）| **PR1 rpc（additive，msg-rpc:9098，不动 dormant rpcgen/message）**：跨域 keystone 例外——SendMessage inline 鉴权（群成员=internal GroupsLogic、媒体=internal mediavalidate）暂依赖 internal；outbox payload 与 message-transfer 消费契约逐字节一致。**PR2 msg-api（additive，纯 BFF:8090，未切流）**：4 消息路由 .api-first→goctl，BFF over gRPC 调 msg-rpc（msgclient + rpcerror.FromStatus）；svc 无 UserRPC。feedback/ai-hosting **不并入 msg-api**（in-process 接线重 + 07 Phase4 注定迁 agent-rpc/feedback-rpc，避免重复搬迁）→ 暂留 message-api。**待**：按 03 §9 重排版（#462）推进——A1 归位 ai-hosting/feedback（#463，ingress /conversations 前缀被 ai-hosting 共享，无法部分切）→ A2 ingress 切流（/messages+/conversations+/api/feedback→msg-api）+ 退休 message-api → A3 gateway-ws 切 msg-rpc gRPC（ACK 语义不变）→ A4 删 internal 消息域（含 rpcgen/message）。写路径 Kafka 化（B0~B3）、拆 push（C）、Redis 读路径/RevokeMessage 等（D，原 07 Phase2-3）随后。|
