...
| agent | goctl（数据层分片退役，AG-6/D13 多 PR；编排已 #340 迁入 agent-rpc）| `service/agent/rpc/internal/convhosting`（业务规则 + Store）| `service/agent/rpc/internal/model` goctl `conversation_ai_hosting_settings`（迁移 024 复合 PK→代理 `id`+`UNIQUE(owner,conversation)` 保 `ON CONFLICT`；partial unique index「单边互斥」不变；custom `FindEnabledByConversationId`/`Upsert`）| #603（AG-6 ①）| **AG-6 ① 仅 `conversation_ai_hosting_settings`**：勘探确认仅 agent-rpc 消费（msg-api 经 gRPC 调，不 import internal）→ **整片删 internal**（非 shim）：`internal/repository/conversation_ai_hosting_*`+`postgres_conversation_ai_hosting.go`+`internal/logic/aihostinglogic*` 与 `postgres_common` 入口已删；`ConversationAIHostingLogic`（含 `AgentAccountExistenceChecker`、`singleConversationParticipants`）搬入 `convhosting`，背靠 Store（model-backed prod / memory fixture）；重接 4 处消费——trigger `hosting.Store`、gRPC CRUD `logic`、orchestrator request builder + `ConversationHostingService`。**剩余/后续 AG-6 ②③…**：agent registry/definition（`agents`+`agent_prompts/tools/skills`+bindings）、audit（`agent_runs/...`）、`agent_conversation_hosting`、MessageLogic 等 message keystone 本 PR 不动，随后续 AG-6/message 迁移删。本 PR **不关闭 #344**（删整个 internal 仍依赖剩余 keystone）。|
