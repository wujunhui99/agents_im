...
- **#463 落地（03 §9 A1+A2 合并）**：message-api/message-rpc 已退役，msg-api 切流接管全部 HTTP 路由。msg.proto 在 10-RPC 之外**临时**新增 `GetConversationAIHosting`/`UpdateConversationAIHosting`（keystone 例外——web 走 REST 发消息，AI 托管触发钩子随 message-api 退役迁入 msg-rpc SendMessage（persist 后 fire，语义对齐原进程内钩子），hosting runtime 接线在 msg-rpc svc。触发点已随 03 §9 B1 迁 msgtransfer（trigger topic 无条件 produce、msg-rpc 内回流 consumer 过渡判定）；这 2 个 RPC 与 hosting runtime 接线待 agent-rpc 上线后迁出——判定分层与迁移路径见 00-decisions **D15** / 04 §4.2）。feedback 创建落 admin-rpc `CreateFeedback`，msg-api BFF 聚合。本文 Phase 4「ai_hosting/feedback 搬走」的 message-api 摘除部分已完成，**最终归属（agent-rpc/feedback-rpc）仍待**。
- **00-decisions D10**：msg-rpc 接口范围以本文为准（10 个 RPC）——已落（#463 临时 +2 个 ai-hosting RPC，见上）。
