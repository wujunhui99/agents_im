...
第一阶段 PostgreSQL migration 已覆盖用户、认证、好友、群聊、消息、outbox、delivery attempt、Agent profile 管理表，以及 Agent prompt/tool/skill registry 元数据表。`users.account_type` 支持 `normal`、`agent`、`admin`，默认 `normal`。
