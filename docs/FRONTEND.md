...
- 从发起聊天搜索结果创建的会话标题优先使用 `display_name / name / identifier`。从消息 API 直接加载的历史会话目前只包含 numeric account ID；前端不会伪造 `/users/id/{user_id}`，因此在后端提供按 user_id 查询公开资料的前端契约前，历史会话标题只能回退显示内部 user ID。
