...
import (
	...
	"github.com/wujunhui99/agents_im/service/msg/api/internal/handler"
...
func main() {
	configFile := flag.String("f", "etc/msg-api.yaml", "the config file")
	flag.Parse()
	run(*configFile)
}

// run starts the msg-api service: it loads config and serves.
...
func run(configFile string) {
	...
	if err != nil {
		log.Fatalf("build msg api service context: %v", err)
	}
...
func registerObservabilityHandlers(server *rest.Server, serviceContext *svc.ServiceContext) {
	server.AddRoutes([]rest.Route{
		{Method: http.MethodGet, Path: "/healthz", Handler: healthHandler("msg-api")},
		{Method: http.MethodGet, Path: "/readyz", Handler: health.ReadinessHandler("msg-api", func(*http.Request) []health.Check {
			return []health.Check{
				componentCheck("auth_config", serviceContext != nil && serviceContext.Config.Auth.AccessSecret != "", "configured"),
				componentCheck("msg_rpc", serviceContext != nil && serviceContext.MsgRPC != nil, "configured"),
			}
		})},
