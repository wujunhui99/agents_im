...
// 单聊看 conversation_id 里的双方；群聊经 groups-rpc ListMembers 鉴权（#617）。
func ensureConversationReadAccess(ctx context.Context, groups groupsrpc.GroupMemberLister, userID, conversationID string) error {
	...
	_, err := groups.ListMembers(ctx, groupsrpc.ListMembersRequest{
		GroupID:         groupID,
...
func filterReadableConversationStates(ctx context.Context, groups groupsrpc.GroupMemberLister, userID string, states []model.ConversationSeqState) ([]model.ConversationSeqState, error) {
