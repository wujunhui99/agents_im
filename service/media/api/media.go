...
import (
	...
	"github.com/wujunhui99/agents_im/service/media/api/internal/handler"
...
func main() {
	configFile := flag.String("f", "etc/media-api.yaml", "the config file")
	flag.Parse()
	run(*configFile)
}

// run starts the media-api service: it loads config and serves.
func run(configFile string) {
	...
	if err != nil {
		log.Fatalf("build media api service context: %v", err)
	}
	...
	server.AddRoutes([]rest.Route{
		{
			...
			Path:    "/healthz",
			Handler: healthHandler("media-api"),
		},
		{
			Method: http.MethodGet,
			Path:   "/readyz",
			Handler: health.ReadinessHandler("media-api", func(*http.Request) []health.Check {
				return []health.Check{
					componentCheck("auth_config", serviceContext != nil && serviceContext.Config.Auth.AccessSecret != "", "configured"),
					componentCheck("media_rpc", serviceContext != nil && serviceContext.MediaRPC != nil, "configured"),
				}
			}),
		},
		{
