// Package userrpc 用 user-rpc 实现 agent-rpc AI 托管运行时所需的 repository.Repository：
// agent-create 工具路径（AgentAssemblyLogic.CreateAgentFromTool）的账号读写经属主 user-rpc，
// 不走顶层 internal/repository accountRepo（执行 profiles.avatar_media_id 的 string scan / 空串写）。
// 是 gate #550（avatar_media_id text→bigint）的延续；与 msg-rpc 同名适配器同源（#340 随 AI
// 托管运行时迁入属主 agent 域，待 internal/ 完全退役后两份一并清理）。
//
// AI 托管运行时（service/agent/rpc/internal/aihosting）把 Accounts 与 Friendships 都取自
...
func unsupported(method string) error {
	return apperror.Internal("agent-rpc user-rpc adapter does not support account write " + method)
}

// toUser 把 user-rpc UserEntity 映射成 pkg/model.User。avatar 维持十进制串（wire 不变）；
...
// fromUnixMilli 把 UnixMilli(UTC) 还原成 time.Time；0 → 零值。
