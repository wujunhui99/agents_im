...
import (
	...
	"github.com/wujunhui99/agents_im/service/friends/api/internal/handler"
...
// Start bridges cmd/friends-api to the service/friends/api goctl-generated API internals.
// cmd/friends-api cannot import service/friends/api/internal/* directly because
...
func Start(configFile string) {
	...
	if err != nil {
		log.Fatalf("build friends api service context: %v", err)
	}
	...
	server.AddRoutes([]rest.Route{
		{
			...
			Path:    "/healthz",
			Handler: healthHandler("friends-api"),
		},
		{
			Method: http.MethodGet,
			Path:   "/readyz",
			Handler: health.ReadinessHandler("friends-api", func(*http.Request) []health.Check {
				return []health.Check{
					componentCheck("auth_config", serviceContext != nil && serviceContext.Config.Auth.AccessSecret != "", "configured"),
					componentCheck("friends_rpc", serviceContext != nil && serviceContext.FriendsRPC != nil, "configured"),
				}
			}),
		},
		{
