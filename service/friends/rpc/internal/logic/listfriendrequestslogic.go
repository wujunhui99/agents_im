...
func (l *ListFriendRequestsLogic) ListFriendRequests(in *friends.ListFriendRequestsRequest) (*friends.ListFriendRequestsResponse, error) {
	result, err := l.svcCtx.FriendsLogic.ListFriendRequests(l.ctx, business.ListFriendRequestsRequest{UserID: in.GetUserId()})
	if err != nil {
		return nil, rpcerror.ToStatus(err)
	}
	incoming := make([]*friends.Friendship, 0, len(result.Incoming))
	for _, item := range result.Incoming {
		incoming = append(incoming, toFriendship(item))
	}
	outgoing := make([]*friends.Friendship, 0, len(result.Outgoing))
	for _, item := range result.Outgoing {
		outgoing = append(outgoing, toFriendship(item))
	}
